	...
	<div class="chord-list" on:click|self={closeEditor}>
		{#each chords as chord, i}
			...
			>
				...
				<button
					class="remove-btn"
					on:click|stopPropagation={() => removeChord(i)}>×</button
				>
			</div>
		{/each}
		...
		<div class="editor-panel">
			<div class="panel-header">
				<span class="edit-title"
					>EDIT CHORD: <span class="highlight"
						>{chords[activeIndex]}</span
					></span
				>
				<button class="close-panel-btn" on:click={closeEditor}
					>DONE</button
				>
			</div>
			...
			<div class="selector-group">
				...
				<div class="grid roots">
					{#each tonics as t}
						<button
							class="chip"
							class:selected={getChordParts(chords[activeIndex])
								.root === t}
							on:click={() =>
								updateActiveChord(
									t,
									getChordParts(chords[activeIndex]).quality
								)}
						>
							{t}
						</button>
				...
				<div class="grid qualities">
					{#each chordQualities as q}
						<button
							...
							on:click={() =>
								updateActiveChord(
									getChordParts(chords[activeIndex]).root,
									q.value
								)}
						>
							{q.label || 'Maj'}
